...
-- RECORD의 제한(전체 데이트 개수 > 조건에 맞는 데이터 개수)
...
-- PK, UNIQUE 로 설정된 칼럼을 '복합 조건절'로 SELECTION할 때는
--      레코드가 1개만 나타날 지라도 이 데이터는 List로 취급한다.
SELECT * FROM tbl_student
WHERE st_num >= '20001' AND st_num <= '20010';

-- st_name에 저장된 값이 독고예준인 리스트만 보여라
-- PK, UNIQUE에 설정된 칼럼이 아닌경우
-- SELECTION을 수행했을 때 나타나는 레코드가 1개 일지라도(독고예준만 나올지라도)
-- 이 데이터는 리스트로 취급한다.
-- ∴ 나중에 java로 바꿀 때 primary key는 VO로 담으면 되지만
-- 다른 칼럼은 반드시 List<VO>에 담아야 한다!
SELECT * FROM tbl_student
WHERE st_name ='독고예준';

-- PROJECTION
-- SELECTION 된 레코드의 개수와 무관하게, 특정 칼럼만 리스트에 포함하는 조회
...
FROM tbl_student
WHERE st_num >= '20001' AND st_num <= '20010';
-- '20001' <= st_num >= '20010' 이렇게 사용하면 안된다.

-- BETWEEN 연산자를 사용하여 범위를 제한하는 SELECTION 구현하기
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
...
-- 한 개의 칼럼에 다수의 OR 조건을 부여하여
-- SELECTION 수행할 때는 IN 연산자를 사용하여 구현할 수 있다.
