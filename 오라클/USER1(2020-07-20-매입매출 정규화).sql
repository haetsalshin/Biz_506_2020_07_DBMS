-- 여기는 User1
-- 현재 tbl_iolist에는 상품명, 거래처명, 거래처CEO 칼럼의 데이터가 일반 문자열 형태로 저장되어 있다.
-- 이 데이터는 같은 칼럼에 중복된 데이터가 있어서 데이터 관리 측면에서 상당히 불편한 상황이다.
--      만약 어떤 상품의 상품명이 변경이 필요한 경우 
--      상품명을 업데이트 하여야 하는데, 2개 이상의 레코드를 대상으로 업데이트 과정이 필요하다.
--      2개 이상의 레코드를 업데이트 수행하는 것은 데이터의 무결성을 해칠 수 있는 수행이 된다.
--      이러한 문제를 방지하기 위해 다음과 같은 정규화를 실행한다.

-- 상품명 정보를 별도의 테이블로 분리하고, 상품정보에 상품코드를 부여한 후
--      tbl_iolist와 연동하는 방식으로 정규화를 수행한다.
...
상품정보 테이블에는 상품코드, 상품 이름, 매입 매출단가가 들어 있다.

매입매출 테이블에 상품이름에 해당하는 상품코드를 매입매출테이블에 UPDATE하고 
매입매출 테이블의 상품이름 칼럼을 제거한 후 JOIN을 수행하여 데이터를 확인
*/

-- 1. 매입매출 테이블의 상품명과 상품정보 테이블의 상품명을 JOIN해서 
--      매입 매출 테이블에 상품명이 상품정보에 모두 있는지 확인 하기
SELECT IO.io_pname, P.p_name
FROM tbl_iolist IO
    LEFT JOIN tbl_product P
        ON IO.io_pname = P.p_name;
        
-- 위 Query를 실행하여 P.p_name 항목에 (null)값이 있다면 상품정보 테이블이 잘못만들어 진것이다.
-- 상품정보 테이블을 삭제하고 과정을 다시 수행해야 한다.

-- Query 결과중에 P.p_name 항목의 값이 null인 리스트만 보여라
-- 쿼리 결과가 정상이라면 리스트는 한개도 없어야 한다.
SELECT IO.io_pname, P.p_name
FROM tbl_iolist IO
    LEFT JOIN tbl_product P
        ON IO.io_pname = P.p_name
WHERE P.p_name IS NULL; -- 해당칼럼의 값이 NULL이 아니냐? : IS NOT NULL

-- 이제 상품데이터가 이상 없음을 알았으니 매입 매출 테이블에 상품 코드를 저장할 "칼럼을 추가"
-- ALTER TABLE : 테이블의 구조(칼럼 추가, 삭제)변경, 칼럼의 타입변경 등을 수행하는 명령
-- 상품 테이블의 p_code와 같은 Type 으로 io_pcode 칼럼을 추가
ALTER TABLE tbl_iolist ADD io_pcode CHAR(5);

ALTER TABLE tbl_iolist DROP COLUMN io_pcode;

-- ALTER TABLE할 때
-- 이미 많은 데이터가 INSERT 되어 있는 상태에서 칼럼을 추가하면
...
-- NOT NULL 조건을 추가하기
-- 1. p_code 칼럼을 추가, 기본값이 문자열이므로빈칸을 추가(숫자 칼럼이면 0을 추가)
...
--          CHAR <==> (n)VARCHAR2는 저장된 문자열이 UNICODE(한글 등)이면 
--          매우 주의를 해야한다.
...
-- 매입매출장의 상품코드(io_pcode) 칼럼에 UPDATE 주기
...
(
    SELECT p_code
    FROM tbl_product P
    WHERE p_name = IO.io_pname -- 여기서 가장 늦게 실행된다. 왜냐하면 IO를 알아야 하니까
);
SELECT io_pcode FROM tbl_iolist;

-- iolist에 pcode가 정상적으로 업데이트 되었는지 검증
SELECT io_pcode, io_pname, p_name
FROM tbl_iolist IO
    LEFT JOIN tbl_product P
...
-- b_tel 칼럼의 값이 중복된(2개 이상) 인 레코드가있냐
...
-- 이 쿼리는 거래처명과 CEO 값을 동시에 제한하여 1개의 row 값만 Sub query에서 만들어 지도록 해야한다.
UPDATE tbl_iolist IO
SET io_bcode =
(
    SELECT b_code
