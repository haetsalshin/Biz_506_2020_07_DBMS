...
-- 외래키(Foreign Key)
-------------------------------------------------
-- 두개(이상)의 테이블간의 Relation을 강제로 설정하여
-- 테이블에 추가되는 데이터의 무결성을 보증하는 제약조건 설정
-- JOIN을 사용하여 두개의 테이블간의 Relataion을 설정하기도 하는데
-- 그 차이점
...
-- 가장 좋은 경우는 테이블을 생성하고 데이터를 한개도 추가 하지 않은 상태로 설정하는 것이다.
-- 하지만 APP을 사용하기 시작하는 초기 단계에서는 다량의 데이터를 추가해야 하는 
-- 일들이 많아서 이 때 FK가 설정이 되어 있으면, Sub Table에 데이터를 추가하는데
-- 많은 애로사항이 발생한다.
-- 이런경우 초기에 FK를 설정하지 않거나, FK 제거하고 데이터를 입력(추가)하도록
-- 하는 경우가 많다.

-- 현재 스키마에 있는 학생정보와 성적정보 테이블을 FK(외래키) 관계로 설정을 한다.
-- FK 관계의 테이블을 부모 테이블과 자식 테이블로 분류하는데, 
-- FK 설정은 자식테이블에서 실시한다.

-- 학생정보 테이블에 학생 정보가 없으면, 성적 정보를 입력할 수 없도록 설정한다.
...
FOREIGN KEY(sc_num)                 --왜래키를(누구를) : sc_num칼럼을 기준으로
REFERENCES tbl_student(st_num);     --부모는 누군데 : tbl_student,
                                    -- 연동(연결)할 부모 칼럼은 어떤 것인데 : st_num
...
-- 확신할 수 있는 방법이 없다.
-- 따라서 이 데이터는 무결성 원칙에 위배되는 데이터 일 수 있다.
...
-- FK 설정이 된 두 테이블간의 관계
------------------------------------------------------------
-- 학생정보 st_num데이터        성적정보 sc_num 데이터
...
-- 아주 특별하게 학생의 학번(st_num)변경 할 경우
--      1. 먼저 성적정보에서 변경할 학번의 성적데이터를 모두 삭제하고
--      2. 학생 정보의 학번을 변경하고
--      3. 변경된 학번으로 성적데이터를 추가해야 한다.

-- ALTER TABLE 명령을 수행하게 되면 명령이 시작된 순간 Table의 모든 데이터 접근이 금지된다.
--      만약 저장된 데이터가 매우 많다면
...
--      RREFERENS : Master Table에 설정을 한다.

-- 학생정보, 학과정보간에 FK를 설정하려고 한다.
-- 학과정보는 학과명(코드)이 유일한 값으로 정해져 있다. 같은 이름의 학과는 존재 할 수 없다.
-- 학생정보에는 학생마다 학과가 다르거나 같을 수 있어서 수없이 많은 중복데이터가 있다.
-- 부모테이블 : 학과정보 1인 경우
-- 자식테이블 : 학생정보중의 학과는 N인 경우

ALTER TABLE tbl_student
ADD CONSTRAINT FK_D_ST -- 한스키마 내에서 이름은 유일해야 한다. 변수처럼!
FOREIGN KEY (st_dept)
