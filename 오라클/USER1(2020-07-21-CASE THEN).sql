...
-- 애석하게도 오라클에는 INCREMENT를 사용할 수 없기 때문에 이렇게 메서드?를 만들어서
-- 하단의 seq_test.NEXTVAL로 호출해서 사용하는 것

-- 쌤) 오라클에는 Table에 AUTO_INCREMENT 설정이 없다.
-- 만약 일련번호와 같은 칼럼을 사용하여 PK로 만들고 싶을 때는 상당히 곤란한 상황이 된다.
-- INSERT를 수행할 때 마다 이전에 저장된 일련번호 값을 조회하고 +1을 수행하여 다시 INSERT를 수행하는 일을
--       반복해야 하기 때문에

-- 다행이 오라클에는 SEQUENCE라는 다른 DBMS에 없는 특별한 객체가 있다.
-- 이 객체는 객체.NEXTVAL 이라는 특별한 호출자가 있는데
-- 이 호출자를 호출할 때마다(SELECT, INSERT등에서) 내부에 가지고 있는 변수값을
-- 사전에 정의한 규칙에 따라 자동 증가하여 보관을 한다.

-- 이 객체.NEXTVAL이 가지고 있는 값을
-- INSERT를 수행할 때 PK인 seq칼럼에 주입을 하면 AUTO_INCREMENT효과를 낼 수 있다.
