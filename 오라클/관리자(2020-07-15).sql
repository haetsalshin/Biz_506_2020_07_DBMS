...
-- 오라클에서 공식적으로 Data를 저장하기 앞서 tableSpace를 생성하라
-- TableSpace를 생성하지 않고 Table 등을 만들고 data를 저장하면
-- 오라클 System 폴더, System TableSpace에 데이터가 저정되어서
...
-- 이러한 이유로 DBA(데이터베이스 관리자) 들도 실무 경험이 많은 사람들이 다루는 분야이다.
-- 그러다보니, 초급(시작하는 DBA)는 별로 다루지 않게 된다.
-- 보통 교재에서는 TableSpace 다루지 않는 경우가 많다.

-- TableSpace 생성에서 고려할 사항
-- 이름, 저장파일, 초기용량, 자동증가옵션
-- 오라클 11gXe는 Maxsize 를 지정하지 않아도 기본값으로 11G로 설정이 된다.
-- 따라서 11gXe를 사용할 때는 최대 저장공간을 11G 이상 사용 할 수 없다.

CREATE TABLESPACE gradeTS
DATAFILE 'C:/bizwork/workspace/oracle_data/gradeTS.dbf'
SIZE 1M AUTOEXTEND ON NEXT 500K;

-- 생성한 TableSpace를 관리하고 Data를 조작할 User생성
-- 사용자 ID, 초기비밀번호, Default TableSpace
