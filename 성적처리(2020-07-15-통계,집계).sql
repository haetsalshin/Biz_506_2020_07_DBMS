...
-- st_name 칼럼에 중복된 값이 없어서 GROUP BY가 의미가 없는 경우도 있다.
SELECT st_name FROM tbl_student
GROUP BY st_name;

-- 데이터를 학과 이름으로 그룹을 짓고 각 학과의 소속된 학생들이 몇명씩인가 알고 싶다.
...
-- st_grade 칼럼으로 GROUP만들고
-- 극 그룹에 속하는 레코드가 몇개인가를 카운트하여 보여준다.
SELECT st_grade, COUNT(*) FROM tbl_student
GROUP BY st_grade;

-- COUNT() *을쓰는 대신 특정한 칼럼을 사용하여도 된다.
...
-- 각 학과의 학생들 중에서 
-- 나이가 가장 많은 학생의 나이와 나이가 가장 적은 학생의 나이를 보여달라
-- 단, 나이가 가장 많은 학생의 이름, 또는 가장 나이가 적은 학생의 이름을 찾는데는
--      이 SQL로는 어렵다.
SELECT st_dept, MAX(st_age), MIN(st_age) FROM tbl_student
GROUP BY st_dept;

-- SQL명령문 : Query(질의) 문 이라고 부른다.
-- 질문 : 정답이 정해져있음. 질의 : 장황하게 설명이 필요한 것
