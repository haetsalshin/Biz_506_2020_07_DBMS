	...
	public static void main(String[] args) {
		...
		try {
			...
			while(result.next()) {
				...
				String o_num = result.getString(DBContract.ORDER.POS_O_NUM_STR);				
				orderVO.setO_num(o_num);
				
				String o_date = result.getString(DBContract.ORDER.POS_O_DATE_STR);
				orderVO.setO_date(o_date);
				
				String o_cnum = result.getString(DBContract.ORDER.POS_O_CNUM_STR);
				orderVO.setO_cnum(o_cnum);
				
				String o_pcode = result.getString(DBContract.ORDER.POS_O_PCODE_STR);
				orderVO.setO_pcode(o_pcode);
				
				String o_pname = result.getString(DBContract.ORDER.POS_O_PCODE_STR);
