...
-- DEFAULT
-- 해당하는 칼럼에 데이터를 지정하지 않고 INSERT를 수행하면
...
-- 학번이 20001~20010dls 학생들만 추출하여 과목별 총점을 계산하라
SELECT SUM(sc_kor) AS 국어총점,
        SUM(sc_eng) AS 영어총점,
        SUM(sc_math) AS 수학총점,
        SUM(sc_music) AS 음악총점,
...
-----------------------------------------------------------------------
-- UNION 하려면 각 모든 연산 칼럼 갯수가 모두 같아야 한다.
-- ==과목총점== 같이 넣어준 칼럼은 반드시 GROUP BY 로 묶어줘야한다.
-----------------------------------------------------------------------
...
-- 통계, 집계 함수를 사용할 때 통계, 집계 함수로 묶이지 않은 칼럼은
